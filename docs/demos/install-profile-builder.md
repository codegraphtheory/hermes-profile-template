...
> The smoke runner validates the generated-profile path first. If Hermes is installed, it then installs this repository as `profile-architect-demo` inside the temporary `HERMES_HOME`; if Hermes is missing, it reports a clear skip instead of failing unrelated validation.
...
> The installed profile turns a short idea into a mature profile prompt and a complete profile repository. The recording should show generated files and validation results, not private runtime state.
